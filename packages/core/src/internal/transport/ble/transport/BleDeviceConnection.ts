...
export class BleDeviceConnection implements DeviceConnection {
  ...
  constructor(
    { device, apduSender, apduReceiver }: BleDeviceConnectionConstructorArgs,
  ...
  private async get writeCharacteristic() {
    const bleServices = await this._device.gatt?.getPrimaryServices() || [];

      const characteristic = service.getCharacteristics()
  }
